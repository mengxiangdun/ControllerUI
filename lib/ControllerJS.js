...
function CreateLayout(node,father) {
    ...
    for (var i=0;i<layout_nodes.length;i++){
        ...
        var cmdInter_nodes=layout_nodes[i].getElementsByTagName("CmdInterfacePoolObject");
        for (var k=0;k<cmdInter_nodes.length;k++){
            var v=cmdInter_nodes[k];
            var son_panel=v.childNodes;
            for (var j=0;j<son_panel.length;j++){
                if (son_panel[j].nodeName==="Panel"){
                    CreatePanel(son_panel[j],document.getElementById(layout_id));
                }
                if (son_panel[j].nodeName==="Accordion"){
                    CreateAccordion(son_panel[j],document.getElementById(layout_id));
                }
                if (son_panel[j].nodeName==="Tabs"){
                    CreateTabs(son_panel[j],document.getElementById(layout_id));
                }
            }
        }

        var view3DInter_nodes=layout_nodes[i].getElementsByTagName("View3DInterfacePoolObject");
        for (var k=0;k<view3DInter_nodes.length;k++){
            var d=view3DInter_nodes[k];
            var ifView3D=d.getAttribute("show");
            if (ifView3D==="true"){

                document.getElementById(layout_id).innerHTML="<div id=\"gameContainer\"  style=\"width: 100%;height: 100%\"></div>\n" +
...
function CreateTabs(node,father) {
    ...
    for (var tab_i=0;tab_i<sonNodes.length;tab_i++){
        if (sonNodes[tab_i].nodeType===1){
            ...
            var a_div=document.createElement("a");
            a_div.setAttribute("href","#"+tabOne_div.id);
            a_div.innerText=(sonNodes[tab_i].getAttribute(("text")));
            // a_div.innerText=ReturnUI_Text(sonNodes[tab_i].getAttribute(("text")));
            // a_div.innerText="123";
            li_div.appendChild(a_div);
            // li_div.innerHTML="<a href="#""+tabs_div.id+"tab-"+(tab_i+1)+">tab-1</a>";
...
function CreateAccordion(node,father) {
    ...
    for (var i=0;i<item_list.length;i++){
        ...
        head_div.innerText=item_node.getAttribute("text");
...
function CreateButton(node,father) {
    ...
    // setTimeout("$( \"#"+btn_cmd.id+"\" ).width(80)",65);
